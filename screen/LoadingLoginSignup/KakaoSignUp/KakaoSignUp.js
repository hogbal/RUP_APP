...
function KakaoSignUp(props) {
  ...
  const [sex, setSex] = useState(
    props.route.params.kakao_account.gender_needs_agreement
      ? null
      : props.route.params.kakao_account.gender === 'MALE'
      ? 'M'
      : 'W',
  );
  const [birth, setBirth] = useState(
    props.route.params.kakao_account.birthday_needs_agreement
      ? null
      : props.route.params.kakao_account.birthday,
  );
  const [nickName, setNickName] = useState(
    props.route.params.properties.nickname,
  );
  const [email, setEmail] = useState(
    props.route.params.kakao_account.email_needs_agreement
      ? null
      : props.route.params.kakao_account.email,
  );
  ...
  const getMainData = uid => {
    ...
    axios
      .get('http://13.124.80.15/home/main', {
        params: {
          uid: uid,
        },
      })
      .then(function (response) {
        const user = {
          ...
          profileImage:
            'https://image.fnnews.com/resource/media/image/2022/07/16/202207160834208420_l.jpg',
          point: 0,
          countRecycle: 0,
  ...
  const isNickCheck = () => {
    if (nickCheck === true) return emailCheck();
    else return showToast('닉네임 중복 확인 해주세요');
  };
  const emailCheck = () => {
    if (validateEmail(email)) return univCheck();
    else {
      console.log(email);
  ...
  const univCheck = () => {
    if (univ !== '학교찾기') return majorCheck();
    else return showToast('학교 선택 해주세요');
  };
  const majorCheck = () => {
    if (major !== '학과') return postSignUp();
    else return showToast('학과 선택 해주세요');
  };
      ...
      <View style={{flex: 1, alignItems: 'center', marginTop: '30%'}}>
        ...
        <View style={{flexDirection: 'row'}}>
          ...
          <TextInput
            ...
            defaultValue={
              k_email ? null : props.route.params.kakao_account.email
            }
        ...
        <View style={{flexDirection: 'row'}}>
          <Image style={{width: 5, height: 5}} source={redStar} />
