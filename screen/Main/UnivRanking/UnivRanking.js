  ...
  const Item = ({item}) => {
    return (
      <TouchableOpacity
        style={dystyle().rank}
        onPress={() =>
          navigation.navigate('PersonalRanking', {univ: item.college})
        }>
        <View style={styles.univLogoContainer}></View>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {searchUnivText && (
        <TouchableOpacity
          onPress={() => {
            setSearchUnivText(false), setSearchUnivBox(true);
          }}
      ...
      <Text
        style={{
          marginLeft: screenWidth * 0.0538461,
          fontSize: 24,
          marginBottom: screenHeight * 0.023696682,
          color: 'black',
          fontWeight: '600',
        }}>
        학교순위
